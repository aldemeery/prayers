const formatRemainingTime = (time) => `${time.hours}`.padStart(2, '0') + '<span class="accent">:</span>' + `${time.minutes}`.padStart(2, '0') + '<span class="accent">:</span>' + `${time.seconds}`.padStart(2, '0');
...
prayers.on('start', (data) => {
    ...
    document.getElementById('countdown').innerHTML = formatRemainingTime(data.remainingTime);
